...
> ⚠️ **`_behavior: "use"` não está entregando resultado correto — não use este controle por ora.**
> Defeito conhecido, nos **dois** caminhos: quando a entrada **não** existe, a resposta vem sem os
> registros **e a consulta sequer é executada**; quando existe, o cliente recebe o **invólucro** da
> entrada em vez do resultado — ou seja, formato **diferente** do da mesma consulta sem cache. O alcance
> é limitado porque o controle é **opt-in**: só afeta quem o declara. **Enquanto não houver correção,
> omita `_cache` ou use `_behavior: "ignore"`** — a consulta sem cache responde corretamente.

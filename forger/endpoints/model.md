...
**Opcional**, em `command.<cmd>` e em `event.<evt>`. É o nome que a tela mostra ao usuário **no lugar da
chave**; ausente, a tela mostra a chave. **Comando e evento sem a chave publicam como antes.**

```json
"command": { "registrar": { "alias": "Registrar pedido", … } },
"event":   { "registrado": { "alias": "Pedido registrado", "type": "registrado", … } }
```

> Ilustrativo: `…` é placeholder, não JSON literal.

Texto livre: acento e espaço são aceitos, e a unicidade não é conferida. Recusas na publicação, todas
`400`: `alias` vazio, só de espaços, ou que não é texto.

A **chave** continua sendo o que o cliente envia no comando e o que o motor usa para achar o evento
(`endState`); o `alias` não a substitui, nem ao `type` do evento, e **o motor não o lê**. Quem lê é o
BFF: ver [bff](../../bff/README.md).
